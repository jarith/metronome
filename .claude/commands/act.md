...
- **Parallel Tasks**: If tasks are independent, launch multiple `react-native-coder` agents in parallel using a SINGLE message with multiple Task tool calls
- **Agent Selection**:
  - Use `react-native-coder` agent for implementation tasks
  - If an agent needs to write tests, it should invoke the `test-engineer` agent
...
1. Launch the `react-native-coder` agent again with:
...
2. After fixes, run the solution-reviewer again
...
2. Create a new markdown file: `.claude/task-review-report.md`
...
- **If test writing is needed**, the implementing agent should delegate to test-engineer
