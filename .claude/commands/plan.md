...
- ✅ Write markdown files to the `tasks/` folder
...
1. **Read REQUIREMENTS.md**: Understand the project's core requirements
   ```
...
3. **Explore Codebase**: Use the Task tool with `subagent_type=Explore` to understand:
...
- Examples: `001-add-asset-import-detection.md`, `002-implement-type-resolution.md`
...
- Related requirements from REQUIREMENTS.md (with line references)
...
- **Signature**: `const functionName = (param: Type): ReturnType => {}`
...
- REQUIREMENTS.md:{line-numbers}
- Related file: `path/to/file.ts:{line}`
- Related task: Task {number}
```

### 5. Summary Document Creation

After creating all task files, create `tasks/PLAN.md`:
...
```
/act
I want to execute the tasks in tasks/
```

## Files Created
- tasks/PLAN.md - Overall execution plan
- tasks/001-{name}.md - {brief description}
- tasks/002-{name}.md - {brief description}
...
```
...
- [ ] All references to REQUIREMENTS.md include line numbers
...
- **Reference Requirements**: Always cite specific line numbers from REQUIREMENTS.md
- **Consider Testing**: Every task should have clear testing requirements
- **Plan for Agents**: Write as if explaining to another experienced developer who can't ask questions
